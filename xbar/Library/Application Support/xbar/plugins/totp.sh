...
# <xbar.dependencies>shell,oathtool</xbar.dependencies>
...
# Load TOTP seeds from config file
...
# Handle copy action - generate fresh token on-demand when clicked
if [[ "$1" == "copy" ]]; then
  # $2 contains the base32 secret
  fresh_token=$(get-totp "$2")
...
# Display only account names - token generated fresh on click
for seed in "${seeds[@]}" ; do
    KEY="${seed%%:*}"
    VALUE="${seed##*:}"
    # Pass the secret (VALUE) so fresh token is generated when user clicks
