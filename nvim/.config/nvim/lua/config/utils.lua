...
M.toggle_go_test = function()
	...
	if current_file == "" or not current_file:match("%.go$") then
	...
	if current_file:match("_test%.go$") then
		...
		local non_test_file = current_file:gsub("_test%.go$", ".go")
		if vim.fn.filereadable(non_test_file) == 1 then
			-- Open the corresponding non-test file if it exists
			vim.cmd.edit(vim.fn.fnameescape(non_test_file))
...
vim.api.nvim_create_user_command(
	"CopyLineNumbers",
	M.get_highlighted_line_numbers,
